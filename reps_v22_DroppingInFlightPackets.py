...
class NetworkSimulation:
    ...
    def update_link_bw(self, u, v, bw):
        ...
        with self.lock:
            ...
            # --- AGGRESSIVE FLUSH LOGIC ---
            # If a link goes to 0 (Failed), immediately kill all packets currently on that link.
            # This prevents visual "stuck" packets or delay in reaction.
            if new_bw == 0:
                for p in self.packets:
                    if p.get('drop', False): continue  # Already dropping

                    # Check if packet is currently traversing this link
                    if p['hop'] < len(p['path']) - 1:
                        curr_u = p['path'][p['hop']]
                        curr_v = p['path'][p['hop'] + 1]

                        # Match link (undirected match)
                        if tuple(sorted((curr_u, curr_v))) == key:
                            # Kill it immediately
                            self.dropped_packets_count += 1
                            p['drop'] = True
                            p['drop_timer'] = DROP_ANIMATION_FRAMES
                            p['pct'] = 1.0  # Snap to end for visual clarity of drop point
                ...
                if p['pct'] >= 1.0:
                    ...
                    if link_failed:
                        if self.use_deflection_mode:
                            ...
                            # 2. Add current failure to known bad edges
                            p['avoid_edges'].append(tuple(sorted((curr_node_id, next_id))))
                            p['deflected'] = True
                            p['deflection_count'] += 1
                            self.deflected_packets_count += 1

                            # 3. Pathfinding Loop (Backtrack if needed)
                            found_path = False
                            backtrack_steps = 0

                            while True:
                                search_idx = p['hop'] - backtrack_steps

                                # SAFETY CHECK: Did we hit the Source Host?
                                if search_idx < 0:
                                    break

                                search_node_id = p['path'][search_idx]

                                if search_node_id == p['src']:
                                    break

                                # Try to find path from search_node_id to dst avoiding ALL known bad edges
                                temp_graph = self.graph.copy()
                                for u_avoid, v_avoid in p['avoid_edges']:
                                    if temp_graph.has_edge(u_avoid, v_avoid):
                                        temp_graph.remove_edge(u_avoid, v_avoid)

                                try:
                                    new_sub_path = nx.shortest_path(temp_graph, source=search_node_id, target=p['dst'])
                                    valid_history = p['path'][:p['hop'] + 1]

                                    if backtrack_steps > 0:
                                        retracing_segment = []
                                        for k in range(backtrack_steps):
                                            idx = p['hop'] - 1 - k
                                            retracing_segment.append(p['path'][idx])

                                        full_new_path = valid_history + retracing_segment + new_sub_path[1:]
                                        p['backtracking'] = True
                                    else:
                                        full_new_path = valid_history + new_sub_path[1:]
                                    ...
                                    backtrack_steps += 1
                                    if search_idx > 0:
                                        u_prev = p['path'][search_idx - 1]
                                        v_curr = p['path'][search_idx]
                                        p['avoid_edges'].append(tuple(sorted((u_prev, v_curr))))
...
# Split HTML to avoid large string literals causing errors
HTML_PART_1 = """
<!DOCTYPE html>
<html>
<head>
        ...
        function draw() {
            ...
            state.packets.forEach(p => {
                ...
                if (p.drop) {
                    ...
                    if (p.deflect_anim > 0) {
                        scale = 1.8 + (Math.sin(p.deflect_anim) * 0.5); // BIGGER FLASH
                        bgColor = p.backtracking ? '#e67e22' : '#ff00ff'; // Orange for backtracking, Pink for forward
                    }
                    else if (p.ack) bgColor = '#27ae60';
                ...
                if (p.drop) {
                    ...
                    ctx.fillText(p.deflected ? (p.backtracking ? 'BACK' : 'DEFL') : `E:${p.ev}`, 0, 0);
