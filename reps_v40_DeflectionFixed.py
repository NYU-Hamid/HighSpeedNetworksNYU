                ...
                if p['pct'] >= 1.0:
                    ...
                    if not link_alive or queue_full:
                        if self.use_deflection_mode and p['deflection_count'] < 3:
                            ...
                            candidates = []
                            prev_hop_id = p['path'][p['hop']]  # Avoid backtracking (Source of ingress)

                            for n in neighbors:
                                # Filter 1: Exclude Ingress Node (Prevent simple ping-pong)
                                if n == prev_hop_id: continue

                                # Filter 2: Link Active
                                n_key = tuple(sorted((arrival_node_id, n)))
                                is_alive = False
                                for l in self.links:
                                    if l['key'] == n_key and l['bw'] > 0: is_alive = True; break

                                # Filter 3: Queue Not Full (Lookahead)
                                n_queue_full = False
                                if isinstance(arrival_obj, REPSSwitch):
                            ...
                            def priority_key(n):
                                ...
                                # Tie-breaker: Distance to dest using Safe Graph (Avoiding current node)
                                dist = 9999
                                try:
                                    temp_g = self.graph.copy()
                                    if temp_g.has_node(arrival_node_id): 
                                        temp_g.remove_node(arrival_node_id) # Simulate path from Neighbor assuming it CANT come back here
                                    dist = nx.shortest_path_length(temp_g, n, p['dst'])
                                except:
                                    pass
                            ...
                            if candidates:
                                ...
                                if isinstance(arrival_obj, REPSSwitch):
                                    ...
                                    try:
                                        ...
                                        # Strict Loop Prevention: Remove the current node from the graph for calculation
                                        # This forces the neighbor to find a path that does NOT come back to us.
                                        temp_graph = self.graph.copy()
                                        if temp_graph.has_node(arrival_node_id):
                                            temp_graph.remove_node(arrival_node_id)
                                        
                                        # Also remove ingress to be safe (Strict Exclude Ingress)
                                        if temp_graph.has_node(prev_hop_id):
                                            temp_graph.remove_node(prev_hop_id)
