...
NUM_PKTS_BDP = 100
FREEZING_TIMEOUT_TICKS = 3
EVS_SIZE = 65536  # Standard 16-bit entropy size (Paper Section 2.2)
...
class REPSSwitch:
    def __init__(self, id, layer, capacity):
        ...
        # We ensure it is at least 1 packet to prevent instant dropping in small queues
    ...
    def check_ecn(self, next_hop):
        # Paper Section 2.1: Mark ECN if queue length > K_min
        return self.get_load(next_hop) > self.ecn_threshold


class REPSHost:
    """
    Implements REPS Host logic strictly according to the paper.
    References: Algorithm 1 (ACK/Failure) and Algorithm 2 (Send).
    """
    ...
    def __init__(self, id):
        ...
        self.head = 0
        self.num_valid_evs = 0  # Explicitly tracks 'numberOfValidEVs' from Alg 1/2
        self.is_freezing_mode = False
        self.explore_counter = NUM_PKTS_BDP  # 'exploreCounter'
    ...
    def get_next_ev(self):
        # Algorithm 2 Line 15: Condition to explore random EV
        # 1. Buffer empty (implicitly handled by num_valid_evs check)
        # 2. OR (No valid EVs AND Not in Freezing Mode)
        # 3. OR (Still in initial Exploration Phase)
        should_explore = (self.explore_counter > 0) or \
                         (self.num_valid_evs == 0 and not self.is_freezing_mode)

        if should_explore:
            # Algorithm 2 Lines 16-17: Random exploration
            ev = random.randint(0, EVS_SIZE - 1)
            self.explore_counter = max(self.explore_counter - 1, 0)
            self.explored_count += 1
            return ev
        else:
            # Algorithm 2 Line 19: Call getNextEV() procedure
            return self._get_next_ev_procedure()

    def _get_next_ev_procedure(self):
        # Algorithm 2 Lines 2-12: getNextEV()

        if self.num_valid_evs > 0:
            # Algorithm 2 Lines 3-6: Reuse oldest valid EV
            # Offset calculation finds the oldest added entry that is still valid
            offset = (self.head - self.num_valid_evs) % REPS_BUFFER_SIZE

            ev = self.buffer[offset]['ev']

            # Mark as invalid after use (Consume the entropy)
            ...
            # Algorithm 2 Lines 7-10: Freezing Mode Cyclic Reuse
            # We are here because num_valid_evs == 0 BUT we are in freezing mode (checked in get_next_ev)
            # We reuse whatever is in the buffer cyclically, ignoring valid bits
            offset = self.head
            self.head = (self.head + 1) % REPS_BUFFER_SIZE

            ev = self.buffer[offset]['ev']

            # Fallback for safety if buffer was totally empty when freezing started
            if ev is None:
                ev = random.randint(0, EVS_SIZE - 1)
    ...
    def on_ack(self, ev, ecn):
        # Algorithm 1: onAck procedure

        # Algorithm 1 Lines 6-8: If ECN is set, return (discard)
        if ecn:
            self.discarded_count += 1
            return

        # Algorithm 1 Lines 9-11: If overwriting an invalid entry, increment count
        # (This means we are adding a new valid EV without overwriting an existing valid one)
        if not self.buffer[self.head]['valid']:
            self.num_valid_evs = min(self.num_valid_evs + 1, REPS_BUFFER_SIZE)

        # Algorithm 1 Lines 12-13: Cache the EV and set valid
        self.buffer[self.head]['ev'] = ev
        self.buffer[self.head]['valid'] = True

        # Algorithm 1 Line 14: Advance head
        self.head = (self.head + 1) % REPS_BUFFER_SIZE

    def on_failure_detection(self, current_tick):
        # Algorithm 1 Lines 21-26: onFailureDetection
        # Only enter freezing if not already there and not in initial exploration
    ...
    def check_and_exit_freezing(self, current_tick):
        # Algorithm 1 Lines 15-18: Check timer to exit freezing mode
        if self.is_freezing_mode and self.freezing_exit_tick and current_tick >= self.freezing_exit_tick:
            self.is_freezing_mode = False
            self.explore_counter = NUM_PKTS_BDP  # Reset exploration counter
...
class NetworkSimulation:
    def __init__(self):
        ...
        self.dropped_packets_count = 0  # NEW: Global drop counter
        self.reset_topology()
    ...
    def reset_topology(self, config=None):
        with self.lock:
            ...
            self.pkt_ctr = 0
            self.running = False
            self.dropped_packets_count = 0  # NEW: Reset drop counter
        ...
        with self.lock:
            ...
            for node in self.nodes.values():
                if isinstance(node, REPSHost):
                    node.check_and_exit_freezing(self.current_tick)
                    ...
                    if isinstance(next_obj, REPSSwitch):
                        if p['hop'] + 2 < len(p['path']):
                            ...
                            if not next_obj.enqueue(outgoing):
                                self.dropped_packets_count += 1  # NEW: Increment drop counter
                                p['drop'] = True
                                p['drop_timer'] = DROP_ANIMATION_FRAMES
                                p['pct'] = 1.0

                                # TRIGGER FREEZING MODE at sender (Failure Detection)
    ...
    def get_state(self):
        with self.lock:
            ...
            # NEW: Return drop count in state
...
<body>
    <div id="config-panel">
        ...
        <!-- NEW METRIC: Dropped Packets Counter -->
    ...
    <script>
        ...
        function draw() {
            ...
            // NEW: Update Dropped Packet Counter in UI
