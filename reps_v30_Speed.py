...
class REPSSwitch:
    def __init__(self, id, layer, capacity):
        ...
        self.queues = {}
        # REPS Paper Section 4.1: K_min set to 20% of queue size (approx 1 BDP)
    ...
    def check_ecn(self, next_hop):
        # Deterministic Marking at Threshold (Simpler than RED for simulation)
...
class REPSHost:
    def __init__(self, id):
        self.id = id
        # Circular Buffer for EV Caching
        ...
        self.inflight_packet = None  # STOP-AND-WAIT Control
    ...
    def get_next_ev(self):
        """
        Determines the next Entropy Value (EV) based on REPS logic.
        Algorithm 2 in Paper.
        """
        # Condition to Explore: Buffer empty OR No valid EVs (and not freezing) OR Initial Warmup
    ...
    def on_ack(self, ev, ecn, d_bit, current_tick):
        """
        Handles incoming ACKs.
        Algorithm 1 in Paper.
        """
        self.inflight_packet = None  # Clear inflight flag to allow next packet

        if d_bit:
            self.deflected_ack_count += 1
            return  # Deflected packets generally not cached in standard REPS

        if ecn:
            # Congestion Detected: Do NOT Cache EV
            self.discarded_count += 1
            return

        # No Congestion: Cache the Good EV
        # "repsBuffer[head].cachedEV <- ackPacket.ev"
        # "repsBuffer[head].isValid <- true"
        ...
        self.head = (self.head + 1) % REPS_BUFFER_SIZE  # Move head circular

        # Exit freezing mode if we get good ACKs
        if self.is_freezing_mode:
            self.is_freezing_mode = False
            self.explore_counter = NUM_PKTS_BDP

    def on_failure_timeout(self):
        """
        Simulates REPS reaction to timeout (potential failure).
        """
        self.inflight_packet = None  # Assume lost, allow retry/next
        if not self.use_deflection_mode:
            self.is_freezing_mode = True  # Enter Freezing Mode
...
class NetworkSimulation:
    def __init__(self):
        ...
        self.speed_multiplier = 1.0  # Controls visual speed only
        self.current_tick = 0
        self.use_deflection_mode = False
        self.reset_topology()
                    ...
                    if p['hop'] < len(p['path']) - 1:
                        ...
                        if tuple(sorted((curr_u, curr_v))) == key:
                            ...
                            if p['src'] in self.nodes and isinstance(self.nodes[p['src']], REPSHost):
                                self.nodes[p['src']].on_failure_timeout()

    def step(self):
        with self.lock:
            self.current_tick += 1

            # 1. Flow Generation (STRICT STOP-AND-WAIT)
            src_id = "H1_1_1"
            dst_id = "H2_2_2"
            ...
            if self.nodes.get(src_id) and self.nodes.get(dst_id):
                ...
                # ONLY send if no packet is currently inflight
                if src_node.inflight_packet is None:
                    ...
                    try:
                        ...
                        if all_paths:
                            ...
                            src_node.inflight_packet = new_pkt['id']  # Mark Inflight
            ...
            # 3. Move Packets
            alive = []
            base_speed = 0.05  # Slightly faster base speed for clarity
            ...
            for p in self.packets:
                ...
                if p['pct'] >= 1.0:
                    ...
                    if p['hop'] == len(p['path']) - 1:
                        if p['type'] == 'DATA':
                            ...
                            if p['dst'] in self.nodes:
                                self.nodes[p['dst']].on_ack(p['ev'], p['ecn'], p['D'], self.current_tick)
                        continue
                    ...
                    if link_failed:
                        ...
                        # Signal Loss/Timeout to Sender
                        if p['src'] in self.nodes and isinstance(self.nodes[p['src']], REPSHost):
                            self.nodes[p['src']].on_failure_timeout()
                        alive.append(p)
                        continue

                    # --- QUEUE MANAGEMENT (TAIL DROP & ECN) ---
...
<html>
<head>
    <title>REPS Simulator: Stop-and-Wait</title>
