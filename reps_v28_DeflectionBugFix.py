        ...
        with self.lock:
            ...
            for p in self.packets:
                ...
                if p['pct'] >= 1.0:
                    ...
                    if p['hop'] == len(p['path']) - 1:
                        if p['type'] == 'DATA':
                            ...
                            if p['dst'] in self.nodes:
                                self.nodes[p['dst']].on_ack(p['ev'], p['ecn'], p['D'],
                                                            self.current_tick)  # <--- Pass D bit
                        continue
                    ...
                    # --- LINK CHECK (Exact logic from paste.txt) ---
                    link_failed = False
                    ...
                    if link_failed:
                        # KEY FIX: Always dequeue from current switch if we are attempting to leave!
                    ...
                    # --- OVERFLOW GUARD (Exact logic from paste.txt) ---
                    if isinstance(next_obj, REPSSwitch):
                        if p['hop'] + 2 < len(p['path']):
                            ...
                            if outgoing_link_failed and not self.use_deflection_mode:
