    ...
    <script>
        ...
        function draw() {
            ...
            state.links.forEach(l => {
                ...
                // SPECIAL HANDLING FOR HOST LAYER
                // Interpolation often fails for hosts because they are the absolute bottom.
                // We simply place the label 40px above the host's exact center.
                if (lowerNode.type === 'host') {
                    targetX = sourcePoint.x;
                    targetY = sourcePoint.y - 40; // Fixed 40px above host
                } else {
                    // STANDARD INTERPOLATION FOR UPPER LAYERS
                    const FIXED_RISE = 45; 
                    targetY = sourcePoint.y - FIXED_RISE;
