...
class REPSSwitch:
    ...
    def check_ecn(self, next_hop):
        # Deterministic ECN marking if queue > 20%
...
class NetworkSimulation:
    def __init__(self):
        ...
        self.flow_rate = 1  # Default 1 packet per tick
        self.current_tick = 0
        self.use_deflection_mode = False
        self.reset_topology()
    ...
    def step(self):
        with self.lock:
            ...
            # 1. Flow Generation (Controlled by flow_rate)
            src_id = "H1_1_1"
            dst_id = "H2_2_2"

            # --- NEW LOOP: Generate exact number of packets requested ---
            if self.nodes.get(src_id) and self.nodes.get(dst_id):
                for _ in range(self.flow_rate):
                    ...
                    if link_failed:
                        ...
                        if self.use_deflection_mode and p['deflection_count'] < 3:
                            ...
                            try:
                                ...
                                if new_paths:
                                    new_path = new_paths[p['ev'] % len(new_paths)]
                                    p['path'] = new_path
                                    p['hop'] = 0
                                    p['pct'] = 0.0
                                    alive.append(p)
...
<body>
    ...
    <div id="controls">
        <div class="slider-container">
             <label style="font-weight:bold; color:#2980b9;">Flow Rate:</label>
             <input type="range" min="1" max="10" step="1" value="1" id="flow-rate-slider" oninput="updateFlowRate(this.value)">
             <span id="flow-rate-val" style="font-weight:bold; color:#2980b9; margin-left:5px;">1</span> pkts/tick
        </div>
        <div class="slider-container">
...
HTML_PART_2 = """
    ...
    <script>
        ...
        function updateFlowRate(val) {
            document.getElementById('flow-rate-val').innerText = val;
            emit('set_flow_rate', val);
        }
...
def handle_ctrl(d):
    ...
    elif d['cmd'] == 'set_flow_rate':
        sim.set_flow_rate(d['val'])
