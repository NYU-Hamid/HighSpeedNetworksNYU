...
NUM_PKTS_BDP = 100
EVS_SIZE = 65536
SEND_GAP_TICKS = 15  # Gap between individual packets (approx 0.75s)
...
class REPSHost:
    ...
    def get_next_ev(self):
        num_valid = sum(1 for entry in self.buffer if entry['valid'])

        if num_valid < REPS_BUFFER_SIZE:
            ev = random.randint(0, EVS_SIZE - 1)
            self.explored_count += 1
            return ev
        else:
            start_ptr = self.reuse_ptr
            chosen_ev = None
            for i in range(REPS_BUFFER_SIZE):
                idx = (start_ptr + i) % REPS_BUFFER_SIZE
                if self.buffer[idx]['valid']:
                    chosen_ev = self.buffer[idx]['ev']
                    self.reuse_ptr = (idx + 1) % REPS_BUFFER_SIZE
                    break
    ...
    def on_ack(self, pkt_id, ev, ecn, d_bit, current_tick):
        ...
        if ecn:
            ...
            for i in range(REPS_BUFFER_SIZE):
                if self.buffer[i]['valid'] and self.buffer[i]['ev'] == ev:
                    self.buffer[i]['valid'] = False
                    self.buffer[i]['ev'] = None
            return
        ...
        if not already_cached:
            insert_idx = -1
            for i in range(REPS_BUFFER_SIZE):
                if not self.buffer[i]['valid']:
                    insert_idx = i
                    break

            if insert_idx == -1:
                insert_idx = self.head
                self.head = (self.head + 1) % REPS_BUFFER_SIZE

            self.buffer[insert_idx]['ev'] = ev
            self.buffer[insert_idx]['valid'] = True

    def on_failure_timeout(self, pkt_id):
        if pkt_id in self.inflight_packets:
            self.inflight_packets.remove(pkt_id)
    ...
    def get_entropy_info(self):
        ...
        for i in range(REPS_BUFFER_SIZE):
            ...
            buffer_entries.append({
                ...
                'is_head': (i == self.head), 'is_reuse': (i == self.reuse_ptr)
            })
        return {
            'id': self.id, 'buffer_entries': buffer_entries,
            'num_valid': sum(1 for e in self.buffer if e['valid']),
            ...
            'explore_counter': 0, 'is_freezing_mode': self.is_freezing_mode,
                    ...
                    if p['hop'] < len(p['path']) - 1:
                        ...
                        if tuple(sorted((curr_u, curr_v))) == key:
                            ...
                            if p['src'] in self.nodes and isinstance(self.nodes[p['src']], REPSHost):
                                self.nodes[p['src']].on_failure_timeout(p['id'])
    ...
    def step(self):
        with self.lock:
            ...
            if self.nodes.get(src_id) and self.nodes.get(dst_id):
                ...
                # Use IF, not WHILE to throttle sending speed (1 packet per tick MAX)
...
<html>
<head>
    <title>REPS Throttled Simulation</title>
