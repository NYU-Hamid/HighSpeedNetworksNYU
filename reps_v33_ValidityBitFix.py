...
class REPSHost:
    ...
    # --- ALGORITHM 1: ONACK() (FIXED: NO DUPLICATES) ---
    def on_ack(self, pkt_id, ev, ecn, d_bit, current_tick):
        ...
        if ecn:
            ...
            # --- FIX START: CHECK FOR EXISTING EV ---
        # Before inserting at head, check if this EV is already in the buffer
        existing_index = -1
        ...
        if existing_index != -1:
            # EV ALREADY EXISTS: Just mark it valid if it wasn't
            ...
            # Do NOT advance head, do NOT overwrite another slot
        else:
            # NEW EV: Proceed with standard insertion logic
                ...
                if p['pct'] >= 1.0:
                    ...
                    if link_failed:
                        ...
                        if self.use_deflection_mode and p['deflection_count'] < 3:
                            ...
                            failed_edges = [l['key'] for l in self.links if l['bw'] == 0]
                            temp_graph.remove_edges_from(failed_edges)
                            try:
                                new_paths = list(nx.all_shortest_paths(temp_graph, curr_node_id, p['dst']))
                                if new_paths:
                                    p['path'] = new_paths[p['ev'] % len(new_paths)]
                                    p['hop'] = 0;
                                    p['pct'] = 0.0
                                    alive.append(p);
                                    continue
...
<html>
<head>
    <title>REPS Unique EV Fix</title>
...
HTML_PART_2 = """
    ...
    <script>
            ...
            state.packets.forEach(p => {
                ...
                if (p.drop) {
                    ...
                    if (p.deflect_anim > 0) {
                        ...
                        scale = 1.0 + Math.sin(anim_prog * 3.14) * 0.8; 
                        bgColor = '#e67e22'; // Flash Orange on deflection
                    }
                    else if (p.ack) {
                        if (p.D) bgColor = '#8e44ad'; // Purple ACK for Deflected (D-bit set)
                ...
                if (p.drop) {
                    ctx.font = "bold 9px Consolas"; ctx.fillText("DROP", 0, 0);
                } else {
                    const idStr = p.ack ? `A:${p.acked_id%100}` : `P:${p.id%100}`;

                    ctx.fillText(idStr, 0, -18);        
                    ctx.fillText(`E:${p.ev}`, 0, -6);   
                    ctx.fillText(`C:${p.ecn?1:0}`, 0, 6); 
        ...
        function drawLabelAt(lx, ly, util, bw) {
            ...
            if (bw === 0) { ctx.globalAlpha = 0.5; }
            ctx.fillStyle = 'rgba(255,255,255,0.95)'; ctx.fillRect(x-22, y-10, 44, 20); 
            ctx.strokeStyle = borderColor; ctx.lineWidth=1; ctx.strokeRect(x-22, y-10, 44, 20);
