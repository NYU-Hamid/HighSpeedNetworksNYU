        ...
        with self.lock:
            ...
            for p in self.packets:
                try:
                    ...
                    # If dropped, we want it to appear at the node it failed to enter
                    # Because we handled drop in the >=1.0 block, the 'u' and 'v' logic
                    # depends on whether we incremented hop or not.
                    # Inside the drop block, we did NOT increment hop.
                    # So 'u' is previous node, 'v' is the node that dropped it.
                    # pct is 1.0. So the packet is drawn at 'v'.
    ...
    <script>
        ...
        function draw() {
            ...
            state.packets.forEach(p => {
                ...
                if (p.drop) {
                    const MAX_DROP_FRAMES = 30; // Must match backend constant
                    const progress = (MAX_DROP_FRAMES - p.drop_timer) / MAX_DROP_FRAMES; 

                    // Fall Downwards (Y-axis offset)
                    const dropDist = 60; 
                ...
                if (p.drop) {
                    // Simpler text for dropping packet
        ...
        function drawLabelAt(lx, ly, util, bw) {
            ...
            // REVERTED to subtle grey border to match original look
            // Only utilization text changes
            ...
            // FIXED: Show accurate 2-decimal percentage
