    ...
    <script>
        ...
        function draw() {
            ...
            state.links.forEach(l => {
                ...
                // --- SIMPLIFIED LABEL PLACEMENT LOGIC ---
                // For Host-Edge links: Place label near Edge switch (80% from Host toward Edge)
                // For other links: Place label near the lower layer switch (45px above)
                ...
                if (lowerNode.type === 'host') {
                    // Host-Edge link: place label at 80% from Host toward Edge (near Edge)
                    const t = 0.80;
                    targetX = lowerPoint.x + (higherPoint.x - lowerPoint.x) * t;
                    targetY = lowerPoint.y + (higherPoint.y - lowerPoint.y) * t;
                } else {
                    // Edge-Agg or Agg-Core: place label 45px above the lower switch
                    const FIXED_RISE = 45;
