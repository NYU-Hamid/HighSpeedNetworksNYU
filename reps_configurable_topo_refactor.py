...
class REPSSwitch:
    ...
    # ECN usually marks if queue > 50% full
    def check_ecn(self):
...
class NetworkSimulation:
    ...
    def _add_link(self, u, v, uf, us, u_max, vf, vs, v_max):
        ...
        self.links.append({
            ...
            'active': 0,
            'bw': 10,  # Default 10 Gbps
            'util': 0.0  # Current utilization
        })
    ...
    def step(self):
        with self.lock:
            ...
            for l in self.links:
                ...
                # Util Calc: active acts as proxy for instantaneous throughput
    ...
    <script>
        ...
        function draw() {
            ...
            state.links.forEach(l => {
                ...
                // Store click area
                linkBoxes.push({u:l.u, v:l.v, x1:p1.x, y1:p1.y, x2:p2.x, y2:p2.y, bw:l.bw});

                ctx.beginPath(); ctx.moveTo(p1.x, p1.y); ctx.lineTo(p2.x, p2.y);
                if(l.active > 0) { ctx.strokeStyle = '#00BFFF'; ctx.lineWidth = 3; }
                else { ctx.strokeStyle = '#d0d0d0'; ctx.lineWidth = 1.5; }
                ctx.stroke();

                // Port Circles (Restored)
                ...
                ctx.fillStyle = '#fff'; ctx.fillRect(mx-14, my-5, 28, 10);
                ctx.strokeStyle = '#ccc'; ctx.lineWidth=1; ctx.strokeRect(mx-14, my-5, 28, 10);
                ctx.fillStyle = '#000'; ctx.font="8px Arial"; ctx.textAlign="center";
                const uVal = l.util ? parseFloat(l.util).toFixed(1) : "0.0";
                ctx.fillText(uVal+"/"+l.bw+"G", mx, my+3);
            });
            ...
            state.packets.forEach(p => {
                ...
                let p1, p2;
                if(link) {
                    if(link.u==p.u) { p1=getLinkPos(p.u,link.uf,link.us,link.umax); p2=getLinkPos(p.v,link.vf,link.vs,link.vmax); }
                    else { p1=getLinkPos(p.v,link.vf,link.vs,link.vmax); p2=getLinkPos(p.u,link.uf,link.us,link.umax); }
                } else {
