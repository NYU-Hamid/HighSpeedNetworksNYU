...
class REPSHost:
    def __init__(self, id):
        ...
        self.head = 0
        self.reused_count = 0  # Track reused entropy values
        self.explored_count = 0  # Track explored entropy values
    ...
    def get_buffer_info(self):
        """Return entropy buffer information for visualization"""
        valid_evs = [b['ev'] for b in self.buffer if b['valid']]
        all_evs = [b['ev'] for b in self.buffer]
        num_valid = len(valid_evs)
        return {
            'id': self.id,
            'num_valid': num_valid,
            'buffer_size': REPS_BUFFER_SIZE,
            'all_entropy_values': all_evs,
            'valid_entropy_values': valid_evs,
        ...
        with self.lock:
            ...
            if self.nodes.get(src_id) and self.nodes.get(dst_id):
                ...
                if random.random() < 0.40:
                    ...
                    ev, ev_source = src_node.get_next_ev()
                    try:
                        ...
                        self.packets.append({
                            ...
                            'ev': ev, 'ev_source': ev_source, 'ecn': False, 'ack': False,
        ...
        with self.lock:
            ...
            for nid, node in self.nodes.items():
                ...
                if isinstance(node, REPSSwitch):
                    ...
                    d['entropy_info'] = node.get_buffer_info()
                nd[nid] = d
            ...
            for p in self.packets:
                try:
                    ...
                    pd.append({
                        ...
                        'ev': p['ev'], 'ev_source': p.get('ev_source', '?'), 'ack': p['ack'],
...
<head>
    ...
    <style>
        ...
        #entropy-modal { display:none; position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); background:#fff; padding:25px; border-radius:8px; box-shadow:0 10px 30px rgba(0,0,0,0.3); z-index:101; border:1px solid #ccc; max-width:400px; max-height:70vh; overflow-y:auto; }
        ...
        .entropy-list { background:#f9f9f9; padding:15px; border-radius:6px; margin-top:10px; }
        .entropy-item { padding:8px; margin:5px 0; background:#fff; border-left:4px solid #2980b9; font-family:monospace; font-size:12px; }
        .entropy-item.valid { border-left-color:#27ae60; background:#f0f9f6; }
        .entropy-item.invalid { border-left-color:#bdc3c7; background:#f5f5f5; }
        .entropy-label { font-weight:bold; color:#333; }
        .entropy-value { color:#2980b9; font-weight:bold; margin-left:5px; }
        .modal-close { float:right; cursor:pointer; font-size:24px; color:#999; }
        .modal-close:hover { color:#000; }
        .stats-row { display:flex; justify-content:space-between; margin:8px 0; padding:8px; background:#ecf0f1; border-radius:4px; }
    </style>
</head>
<body>
    <div id="config-panel">
    ...
    <div id="entropy-modal">
        ...
        <h3 id="entropy-title">Entropy Buffer</h3>
        <div class="stats-row">
            <span><strong>Valid Entropies:</strong></span>
            <span id="entropy-valid">0/8</span>
        </div>
        <div class="stats-row">
            <span><strong>Head Position:</strong></span>
            <span id="entropy-head">0</span>
        </div>
        <div class="stats-row">
            <span><strong>Reused:</strong></span>
            <span id="entropy-reused">0</span>
        </div>
        <div class="stats-row">
            <span><strong>Explored:</strong></span>
            <span id="entropy-explored">0</span>
        </div>
        <div style="margin-top:15px;">
            <h4>Buffer Contents (Algorithm 1 & 2)</h4>
            <div class="entropy-list" id="entropy-list">
                <!-- Populated by JavaScript -->
            </div>
        </div>
        <p style="font-size:11px; color:#666; margin-top:15px; padding-top:10px; border-top:1px solid #ddd;">
            <strong>REPS Algorithm:</strong> Green = Valid (cached good entropy), Gray = Invalid (already used)<br>
            Buffer reuses valid entropies for good paths. When empty, random exploration begins.
        </p>
    </div>
    ...
    <script>
        ...
        let W, H, state = null;
        let linkHitboxes = [];
        let nodeHitboxes = [];  // Track clickable node areas
        ...
        cvs.addEventListener('click', e => {
            ...
            // Check node hitboxes first (hosts with entropy info)
        ...
        function openEntropyModal(nodeId, entropyInfo) {
            ...
            document.getElementById('entropy-valid').innerText = `${entropyInfo.num_valid}/${entropyInfo.buffer_size}`;
            document.getElementById('entropy-head').innerText = `${entropyInfo.head_position}`;
            document.getElementById('entropy-reused').innerText = `${entropyInfo.reused_count}`;
            document.getElementById('entropy-explored').innerText = `${entropyInfo.explored_count}`;

            // Build entropy buffer visualization
            let html = '';
            for(let i = 0; i < entropyInfo.all_entropy_values.length; i++) {
                const ev = entropyInfo.all_entropy_values[i];
                const isValid = entropyInfo.valid_entropy_values.includes(ev);
                const isHead = (i === entropyInfo.head_position);
                const className = isValid ? 'entropy-item valid' : 'entropy-item invalid';
                const headMarker = isHead ? ' ← HEAD' : '';
                html += `<div class="${className}">
                    <span class="entropy-label">[${i}]</span>
                    <span class="entropy-value">EV: ${ev >= 0 ? ev : 'empty'}</span>
                    <span style="color:#999; font-size:11px;">${isValid ? '✓ Valid' : '✗ Invalid'}${headMarker}</span>
                </div>`;
            }
            document.getElementById('entropy-list').innerHTML = html;
        ...
        function draw() {
            ...
            Object.entries(state.nodes).forEach(([id, n]) => {
                ...
                if(n.type !== 'host') {
                    ...
                    // Store host hitbox for entropy modal
                    nodeHitboxes.push({
                        id: id,
                        cx: cx,
                        cy: cy,
            ...
            // 4. PACKETS - Spread out to avoid shadowing nodes
            const packetsByNode = {};
            state.packets.forEach(p => {
                const key = `${p.u}-${p.v}`;
                if(!packetsByNode[key]) packetsByNode[key] = [];
                packetsByNode[key].push(p);
            });

            Object.entries(packetsByNode).forEach(([key, packets]) => {
                packets.forEach((p, idx) => {
                    const n1=state.nodes[p.u], n2=state.nodes[p.v];
                    const p1={x:n1.x/100*W, y:n1.y/100*H}, p2={x:n2.x/100*W, y:n2.y/100*H};

                    // Spread packets along link to avoid overlap
                    const numPackets = packets.length;
                    const spread = numPackets > 1 ? (idx - (numPackets-1)/2) * 0.05 : 0;

                    const x = p1.x + (p2.x-p1.x)*p.pct + spread; 
                    const y = p1.y + (p2.y-p1.y)*p.pct + spread;
