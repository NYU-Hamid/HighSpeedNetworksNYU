...
# ==============================================================================
#  SCIENTIFIC CONFIGURATION (REPS PAPER - All from Paper)
# ==============================================================================
# Buffer Size 8 (Theorem 5.1: b ≥ 2.4 ln(n), for n≥8 this is safe)
REPS_BUFFER_SIZE = 8
PACKET_SIZE_BYTES = 1500
SIM_TICK_SEC = 0.05
EWMA_ALPHA = 0.1
DROP_ANIMATION_FRAMES = 30  # Duration of drop animation (ticks)
NUM_PKTS_BDP = 100  # Section 3.1: Bandwidth-Delay Product exploration
FREEZING_TIMEOUT_MS = 1.0  # Section 3.2: 1ms timeout for failure recovery
...
class REPSSwitch:
    ...
    def enqueue(self, next_hop):
        ...
        # STRICT TAIL DROP LOGIC: Check if buffer is full
...
class REPSHost:
    def __init__(self, id):
        self.id = id
        # Algorithm 1: Circular buffer with validity bits (Table 1: ~25 bytes per connection)
        self.buffer = [{'ev': -1, 'valid': False} for _ in range(REPS_BUFFER_SIZE)]
        self.head = 0

        # Entropy Statistics (Algorithm 1 & 2 tracking for visualization)
        self.reused_count = 0  # Times Algorithm 2 reused from buffer
        self.explored_count = 0  # Times Algorithm 2 explored random
        self.discarded_count = 0  # Times Algorithm 1 discarded (ECN)
        self.explore_counter = NUM_PKTS_BDP  # Countdown from NUM_PKTS_BDP

        # Freezing Mode (Section 3.2: Failure Mitigation)
        self.is_freezing_mode = False
        self.freezing_entered_time = None
        self.freezing_exit_time = None

    def get_next_ev(self):
        # Algorithm 2, Lines 2-12: Reuse Oldest Valid or Explore
        # "If VALID cached EVs exist: REUSE oldest (entropy recycling!)"
        candidate_idx = -1
        ...
        if candidate_idx != -1:
            # === REUSE PATH (Section 3.1) ===
            ev = self.buffer[candidate_idx]['ev']
            self.buffer[candidate_idx]['valid'] = False  # Mark as used
            self.reused_count += 1
            return ev
        else:
            # === EXPLORE NEW PATH (Section 3.1) ===
            # "Else if NOT freezing mode: EXPLORE random"
            # "Else (freezing mode): REUSE even invalid entries"
            ev = random.randint(10, 99)
            if not self.is_freezing_mode:
                self.explore_counter = max(self.explore_counter - 1, 0)
            self.explored_count += 1
            return ev

    def on_ack(self, ev, ecn):
        # Algorithm 1, Lines 5-19: Cache if no Congestion
        # "If ECN-marked (congestion): DISCARD entropy value"
        # "Else (good path): CACHE entropy as VALID in circular buffer"
        if ecn:
            # === DISCARD (ECN marked - congestion detected) ===
            self.discarded_count += 1
            return

        # === CACHE (good path - no congestion) ===
        self.buffer[self.head] = {'ev': ev, 'valid': True}
        self.head = (self.head + 1) % REPS_BUFFER_SIZE

    def on_failure_detection(self, timestamp):
        # Algorithm 1, Lines 21-26: Enter Freezing Mode
        # "When failure detected: Avoid exploring (might pick failed path)"
        # "Reuse buffer entries instead"
        if not self.is_freezing_mode:
            self.is_freezing_mode = True
            self.freezing_entered_time = timestamp
            # Section 3.2: 1ms timeout
            self.freezing_exit_time = timestamp + (FREEZING_TIMEOUT_MS / 1000.0)

    def check_and_exit_freezing(self, timestamp):
        # Exit freezing mode after timeout (Section 3.2)
        if self.is_freezing_mode and self.freezing_exit_time and timestamp >= self.freezing_exit_time:
            self.is_freezing_mode = False

    def get_entropy_info(self):
        # Return entropy buffer state for visualization (focus on entropy values)
        num_valid = sum(1 for b in self.buffer if b['valid'])
        buffer_entries = []
        for i in range(REPS_BUFFER_SIZE):
            buffer_entries.append({
                'index': i,
                'ev': self.buffer[i]['ev'],
                'valid': self.buffer[i]['valid'],
                'is_head': (i == self.head)
            })
...
class NetworkSimulation:
    ...
    def step(self):
        with self.lock:
            ...
            if self.nodes.get(src_id) and self.nodes.get(dst_id):
                # Rate increased to ensure congestion happens for demo
                if random.random() < 0.40:
                    ...
                    try:
                        ...
                        self.packets.append({
                            ...
                            'drop': False, 'drop_timer': 0,
                            'acked_id': None
                        })
                    except:
                        pass

            # Check freezing mode exits
            for node in self.nodes.values():
                if isinstance(node, REPSHost):
                    node.check_and_exit_freezing(self.sim_time)
            ...
            for p in self.packets:
                # HANDLE DROPPED PACKET LIFECYCLE
                if p['drop']:
                    p['drop_timer'] -= 1
                    # Keep alive until animation timer ends
                    if p['drop_timer'] > 0:
                        alive.append(p)
                    continue
                ...
                # Arrival at Node Logic (Only check drop when packet arrives)
                if p['pct'] >= 1.0:

                    curr_node_id = p['path'][p['hop']]

                    # 1. Check if we reached final destination
                    if p['hop'] == len(p['path']) - 1:
                        # Reached End of Path
                        if p['type'] == 'DATA':
                    ...
                    # 2. Intermediate Hop Logic
                    next_id = p['path'][p['hop'] + 1]
                    curr_obj = self.nodes.get(curr_node_id)  # The node we are leaving
                    next_obj = self.nodes.get(next_id)  # The node we are entering

                    # Dequeue from current switch
                    if isinstance(curr_obj, REPSSwitch):
                        curr_obj.dequeue(next_id)

                    # Enqueue into next switch
                    # THIS IS WHERE DROPS HAPPEN: At the ingress of the next node
                    if isinstance(next_obj, REPSSwitch):
                        # Look ahead for outgoing link logic (standard REPS model)
                        if p['hop'] + 2 < len(p['path']):
                            outgoing = p['path'][p['hop'] + 2]

                            # === TAIL DROP LOGIC ===
                            if not next_obj.enqueue(outgoing):
                                # Drop happens AT the next node (pct=1.0 relative to link)
                                p['drop'] = True
                                p['drop_timer'] = DROP_ANIMATION_FRAMES
                                p['pct'] = 1.0  # Clamp to destination node for visual clarity

                                # TRIGGER FREEZING MODE at sender (Section 3.2)
                                if p['src'] in self.nodes:
                                    sender = self.nodes[p['src']]
                                    if isinstance(sender, REPSHost):
                                        sender.on_failure_detection(self.sim_time)
    ...
    def get_state(self):
        with self.lock:
            ...
            for p in self.packets:
                try:
                    ...
                    pd.append({
                        ...
                        'drop': p.get('drop', False),
                        'drop_timer': p.get('drop_timer', 0)
                    })
                except:
                    pass
            ...
            ld = []
            for l in self.links:
                # SCALING FIX: Treat capacity as 200 Kbps per "Gbps unit" for util calc
                # This ensures the percentage is visible (~12%) with sparse traffic
                # while keeping the label as "10Gbps".
...
<head>
    ...
    <style>
        ...
        #entropy-modal { display:none; position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); background:#fff; padding:20px; border-radius:8px; box-shadow:0 10px 30px rgba(0,0,0,0.3); z-index:101; border:1px solid #ccc; width:420px; max-height:85vh; overflow-y:auto; }
        ...
        .entropy-section { margin-bottom:12px; border-bottom:1px solid #eee; padding-bottom:10px; }
        .entropy-stat { display:flex; justify-content:space-between; padding:4px 0; font-size:11px; }
        .entropy-stat-label { color:#555; font-weight:bold; }
        .entropy-stat-value { color:#2980b9; font-weight:bold; }
        .buffer-entry { padding:6px; margin:4px 0; background:#f9f9f9; border-left:3px solid #ddd; border-radius:3px; font-family:monospace; font-size:10px; }
        .buffer-entry.valid { background:#e8f5e9; border-left-color:#4caf50; color:#2e7d32; }
        .buffer-entry.head { background:#e3f2fd; border-left-color:#2980b9; }
        .freezing-status { padding:10px; margin:10px 0; border-radius:4px; font-weight:bold; font-size:12px; text-align:center; }
        .freezing-status.active { background:#ffebee; color:#c62828; border:2px solid #c62828; }
        .freezing-status.inactive { background:#e8f5e9; color:#2e7d32; border:2px solid #2e7d32; }
        #entropy-title { color:#2980b9; margin:0 0 12px 0; font-size:16px; }
        .section-title { font-size:11px; font-weight:bold; color:#333; margin:10px 0 8px 0; }
    </style>
</head>
<body>
    <div id="config-panel">
    ...
    <div id="entropy-modal">
        <span class="modal-close" onclick="closeModals()">&times;</span>
        <h2 id="entropy-title">Entropy Buffer: Host</h2>

        <div id="freezing-indicator" class="freezing-status inactive">🟢 Freezing Mode: OFF</div>

        <div class="entropy-section">
            <div class="section-title">Algorithm 1: Path Caching (onAck) - Lines 5-19</div>
            <div class="entropy-stat">
                <span class="entropy-stat-label">Valid Entries (cached good paths):</span>
                <span class="entropy-stat-value" id="ent-valid">0/8</span>
            </div>
            <div class="entropy-stat">
                <span class="entropy-stat-label">Discarded (ECN-marked):</span>
                <span class="entropy-stat-value" id="ent-discarded">0</span>
            </div>
        </div>

        <div class="entropy-section">
            <div class="section-title">Algorithm 2: Path Selection (getNextEV) - Lines 2-12</div>
            <div class="entropy-stat">
                <span class="entropy-stat-label">Reused (recycled from buffer):</span>
                <span class="entropy-stat-value" id="ent-reused">0</span>
            </div>
            <div class="entropy-stat">
                <span class="entropy-stat-label">Explored (random new paths):</span>
                <span class="entropy-stat-value" id="ent-explored">0</span>
            </div>
            <div class="entropy-stat">
                <span class="entropy-stat-label">Explore Counter (BDP):</span>
                <span class="entropy-stat-value" id="ent-explore-counter">100</span>
            </div>
        </div>

        <div class="entropy-section">
            <div class="section-title">Buffer State (Circular Array, Size=8)</div>
            <div class="entropy-stat">
                <span class="entropy-stat-label">Head Position:</span>
                <span class="entropy-stat-value" id="ent-head">0</span>
            </div>
            <div class="entropy-stat">
                <span class="entropy-stat-label">Freezing Mode (Sec 3.2):</span>
                <span class="entropy-stat-value" id="ent-freeze-status">OFF</span>
            </div>
            <div class="entropy-stat">
                <span class="entropy-stat-label">Entered Freezing At:</span>
                <span class="entropy-stat-value" id="ent-freeze-time">Never</span>
            </div>
        </div>

        <div class="entropy-section">
            <div class="section-title">Entropy Buffer Contents (8 Entries)</div>
    ...
    <script>
        ...
        function openEntropyModal(nodeId, entropyInfo) {
            ...
            // Freezing Mode Status
            const freezingIndicator = document.getElementById('freezing-indicator');
            if(entropyInfo.is_freezing_mode) {
                freezingIndicator.className = 'freezing-status active';
                freezingIndicator.innerHTML = '🔴 Freezing Mode: ON (Section 3.2)';
                ...
                freezingIndicator.className = 'freezing-status inactive';
            ...
            for(let entry of entropyInfo.buffer_entries) {
                ...
                const evDisplay = entry.ev >= 0 ? entry.ev : '-';
                bufferHtml += `<div class="${classes}">[${entry.index}] EV=${evDisplay}${status}</div>`;
            }
        ...
        function draw() {
            ...
            // 4. COMPACT PACKETS & DROP ANIMATION
            state.packets.forEach(p => {
                ...
                if (p.drop) {
                    ...
                    ctx.translate(x, y);
                    let bgColor = '#2980b9'; // Blue (Default)
                    if (p.ack) bgColor = '#27ae60'; // Green
                    else if (p.ecn) bgColor = '#c0392b'; // Red
