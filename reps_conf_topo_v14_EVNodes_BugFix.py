...
class REPSSwitch:
    ...
    def check_ecn(self, next_hop):
        # FIX: Ensure K_min is at least 1.0 packet.
        # This prevents 100% ECN marking when Queue Size is small (e.g., 2).
        k_min = max(1.0, self.capacity * 0.2)
        return self.get_load(next_hop) > k_min
...
class REPSHost:
    def __init__(self, id):
        ...
        self.head = 0
        self.num_valid_evs = 0  # Algorithm 1 Line 3
        ...
        # Freezing Mode (Section 3.2)
        self.is_freezing_mode = False
        self.freezing_entered_time = None
        self.freezing_exit_tick = None

    def get_next_ev(self):
        # Algorithm 2: Path Selection Logic

        # Line 15: Check if we should explore
        should_explore = (self.num_valid_evs == 0 and not self.is_freezing_mode) or (self.explore_counter > 0)

        if should_explore:
            # Line 16-17: Random Exploration
            ev = random.randint(0, EVS_SIZE - 1)
            self.explore_counter = max(self.explore_counter - 1, 0)
            self.explored_count += 1
            return ev
        else:
            # Line 19: getNextEV() procedure
            if self.num_valid_evs > 0:
                # Line 4: Calculate offset for Oldest Valid EV
                offset = (self.head - self.num_valid_evs) % REPS_BUFFER_SIZE
                ev = self.buffer[offset]['ev']

                # Line 5-6: Invalidate and decrement
                ...
                # Freezing Mode with empty valid set - Cyclic Reuse
                offset = self.head
                self.head = (self.head + 1) % REPS_BUFFER_SIZE
                ev = self.buffer[offset]['ev']

                # Fallback if buffer was never populated
                if ev is None:
                    ev = random.randint(0, EVS_SIZE - 1)
    ...
    def on_ack(self, ev, ecn):
        # Algorithm 1: Packet Received Logic

        # Line 6-8: If ECN set, discard (do not cache)
        if ecn:
            self.discarded_count += 1
            return

        # Line 9-11: Update valid counter if we are filling an invalid slot
        ...
        # Line 14: Advance Head
        self.head = (self.head + 1) % REPS_BUFFER_SIZE

    def on_failure_detection(self, current_tick):
        # Algorithm 1 Line 21-25
                    ...
                    if isinstance(next_obj, REPSSwitch):
                        if p['hop'] + 2 < len(p['path']):
                            ...
                            if not next_obj.enqueue(outgoing):
                                ...
                                # TRIGGER FREEZING MODE at sender
