...
class REPSHost:
    def __init__(self, id):
        ...
        self.head = 0  # Points to insertion point (Algorithm 1)
        self.reuse_ptr = 0  # Points to next EV to reuse (Algorithm 2)
    ...
    def get_next_ev(self):
        """
        REPS Algorithm 2: Path Selection
        Requirement: "Keep exploring new EV values initially till the circular buffer is full."
        """
        # 1. Check strict fullness
        num_valid = sum(1 for entry in self.buffer if entry['valid'])

        # If buffer is NOT full (< 8 valid entries), we MUST Explore.
        if num_valid < REPS_BUFFER_SIZE:
            ...
            for i in range(REPS_BUFFER_SIZE):
                ...
                if self.buffer[idx]['valid']:
                    ...
                    # Advance pointer for next time (Round Robin reuse)
            ...
            if chosen_ev is not None:
                ...
                # Fallback if logic sync issue (should never happen if num_valid check passed)
                return random.randint(0, EVS_SIZE - 1)

    def on_ack(self, ev, ecn, d_bit, current_tick):
        """
        REPS Algorithm 1: Update Buffer
        """
        self.inflight_packet = None
        ...
        if ecn:
            ...
            # Invalidate ANY occurrence of this EV
            for i in range(REPS_BUFFER_SIZE):
        ...
        for i in range(REPS_BUFFER_SIZE):
            if self.buffer[i]['valid'] and self.buffer[i]['ev'] == ev:
                # Already have it, don't waste a slot or overwrite
                return

                # 2. Find Insertion Slot
        # Priority: Fill an INVALID slot first to reach "Full" state
        insert_idx = -1
        ...
        # If no empty slot (Buffer Full), overwrite at Head (Standard REPS rotation)
        if insert_idx == -1:
...
class NetworkSimulation:
    ...
    def step(self):
        with self.lock:
            ...
            # 1. Flow Generation (Stop-and-Wait)
            src_id = "H1_1_1"
            dst_id = "H2_2_2"
            ...
            if self.nodes.get(src_id) and self.nodes.get(dst_id):
                ...
                if src_node.inflight_packet is None:
                    ...
                    try:
                        ...
                        if all_paths:
                            # Strict Consistency: Same EV -> Same Path
                            ...
                            src_node.inflight_packet = new_pkt['id']
...
<html>
<head>
    <title>REPS Corrected Logic</title>
