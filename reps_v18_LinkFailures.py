    ...
    def step(self):
        with self.lock:
            ...
            for node in self.nodes.values():
                if isinstance(node, REPSHost): node.check_and_exit_freezing(self.current_tick)
            ...
            for p in self.packets:
                ...
                if p['hop'] < len(p['path']) - 1:
                    ...
                    for l in self.links:
                        if l['key'] == key:
                            # If bandwidth is 0, speed is 0 (packet stops)
                ...
                if p['hop'] < len(p['path']) - 1:
                    u, v = p['path'][p['hop']], p['path'][p['hop'] + 1]
                    key = tuple(sorted((u, v)))
                    # Only utilize link if packet is actually moving (speed > 0)
                ...
                if p['pct'] >= 1.0:
                    ...
                    if p['hop'] == len(p['path']) - 1:
                        if p['type'] == 'DATA':
                            ...
                            if p['dst'] in self.nodes: self.nodes[p['dst']].on_ack(p['ev'], p['ecn'])
                        continue
                        ...
                        if p['hop'] + 2 < len(p['path']):
                            ...
                            if not next_obj.enqueue(outgoing):
                                ...
                                if p['src'] in self.nodes:
                                    ...
                                    if isinstance(sender, REPSHost): sender.on_failure_detection(self.current_tick)
                                alive.append(p)
                                continue
