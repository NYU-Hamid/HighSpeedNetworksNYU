...
class REPSHost:
    def __init__(self, id):
        self.id = id
        # Algorithm 1 Line 3: Circular buffer with validity bits
        self.buffer = [{'ev': None, 'valid': False} for _ in range(REPS_BUFFER_SIZE)]
        self.head = 0

        # Entropy Statistics (Algorithm 1 & 2 tracking)
        self.reused_count = 0
    ...
    def get_entropy_info(self):
        ...
        for i in range(REPS_BUFFER_SIZE):
            ...
            buffer_entries.append({
                'index': i,
                'ev': entry['ev'],  # Can be None or actual EV value
                'valid': entry['valid'],
                'is_head': (i == self.head)
            })
