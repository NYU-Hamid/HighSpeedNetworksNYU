                ...
                if p['pct'] >= 1.0:
                    ...
                    if link_failed:
                        if self.use_deflection_mode:
                            ...
                            # Deflection Logic (Path Recalculation)
                            temp_graph = self.graph.copy()
                            if temp_graph.has_edge(curr_node_id, next_id):
                                temp_graph.remove_edge(curr_node_id, next_id)
                            for bad_u, bad_v in p['avoid_edges']:
                                if temp_graph.has_edge(bad_u, bad_v):
                                    temp_graph.remove_edge(bad_u, bad_v)

                            try:
                                new_sub_path = nx.shortest_path(temp_graph, source=curr_node_id, target=p['dst'])
                                p['path'] = p['path'][:p['hop'] + 1] + new_sub_path[1:]
                                p['pct'] = 0.0
        ...
        function draw() {
            ...
            Object.entries(state.nodes).forEach(([id, n]) => {
                ...
                if(n.type !== 'host') {
                    ...
                    // --- PORT HOLES (Circular Indicators) ---
                    // Draw 4 distinct circular ports at the bottom
                    const numPorts = 4;
                    const spacing = w / (numPorts + 1);
                    ctx.fillStyle = "#fff";
                    for(let i=1; i<=numPorts; i++) {
                         ctx.beginPath();
                         ctx.arc(cx - w/2 + i*spacing, cy + h/2 - 5, PORT_RADIUS, 0, Math.PI*2);
                         ctx.fill();
                    }
