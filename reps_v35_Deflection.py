...
class REPSHost:
    ...
    def get_next_ev(self):
        if self.num_valid_evs > 0:
            ...
            if self.buffer[offset]['ev'] is None:
    ...
    def on_ack(self, pkt_id, ev, ecn, d_bit, current_tick):
        if pkt_id in self.inflight_packets:
            self.inflight_packets.remove(pkt_id)

        # REPS Logic: Treat D-bit same as ECN (Congestion/Failure Signal)
        # We DO NOT cache the EV, ensuring we avoid this path in future.
        if d_bit:
    ...
    def can_send(self, current_tick):
        if current_tick - self.last_send_tick < SEND_GAP_TICKS:
            return False

        if self.batch_mode:
            if len(self.inflight_packets) == 0 and self.current_batch_sent >= self.batch_size:
                self.current_batch_sent = 0
            if self.current_batch_sent < self.batch_size:
                return True
            return False
        else:
    ...
    def get_entropy_info(self):
        ...
        for i in range(REPS_BUFFER_SIZE):
            ...
            buffer_entries.append({
                'index': i, 'ev': entry['ev'], 'valid': entry['valid'],
                'is_head': (i == self.head)
            })
        return {
            'id': self.id, 'buffer_entries': buffer_entries,
            'num_valid': self.num_valid_evs,
            'head_position': self.head,
            'reused_count': self.reused_count,
            'explored_count': self.explored_count,
            'discarded_count': self.discarded_count,
            'explore_counter': self.explore_counter,
            'is_freezing_mode': self.is_freezing_mode,
            ...
            'inflight': len(self.inflight_packets),
...
class NetworkSimulation:
    ...
    def set_batch_size(self, size_str):
        ...
        with self.lock:
            for n in self.nodes.values():
                if isinstance(n, REPSHost):
                    n.set_batch_params(self.batch_setting)
    ...
    # --- NEW METHOD: DEFLECTION LOGIC ---
    def get_deflection_hop(self, curr_id, intended_next_id, live_links):
        """
        Determines the next hop locally based on link status and Fat-Tree rules.
        """

        def get_type(nid):
            if nid.startswith("C"): return "core"
            if nid.startswith("A"): return "agg"
            if nid.startswith("E"): return "edge"
            return "host"

        curr_type = get_type(curr_id)
        next_type = get_type(intended_next_id)
        neighbors = list(self.graph.neighbors(curr_id))

        # Filter for ONLY live links
        valid_neighbors = []
        for n in neighbors:
            key = tuple(sorted((curr_id, n)))
            link_data = next((l for l in self.links if l['key'] == key), None)
            if link_data and link_data['bw'] > 0:
                valid_neighbors.append(n)

        if not valid_neighbors: return None  # Dead end

        # SCENARIO 3: Agg -> Edge Failure (Downstream blocked)
        # Action: Deflect UP to a Core
        if curr_type == "agg" and next_type == "edge":
            cores = [n for n in valid_neighbors if get_type(n) == "core"]
            if cores: return random.choice(cores)

        # SCENARIO 4: Core -> Agg Failure (Downstream blocked)
        # Action: Reroute to another Agg in the SAME Pod
        if curr_type == "core" and next_type == "agg":
            # intended_next_id looks like "A1_1". Pod ID is "A1"
            target_pod_prefix = intended_next_id.split('_')[0]
            alts = [n for n in valid_neighbors if get_type(n) == "agg" and n.startswith(target_pod_prefix)]
            if alts: return random.choice(alts)

        # SCENARIO 1 & 2: Edge/Agg Upstream Failure
        # Action: Reroute to any other valid Upstream node
        if (curr_type == "edge" and next_type == "agg") or (curr_type == "agg" and next_type == "core"):
            upstream_alts = [n for n in valid_neighbors if get_type(n) == next_type]
            if upstream_alts: return random.choice(upstream_alts)
            ...
            for p in self.packets:
                ...
                if p['pct'] >= 1.0:
                    ...
                    if link_failed:
                        ...
                        # --- UPDATED DEFLECTION LOGIC ---
                        if self.use_deflection_mode and p['deflection_count'] < 3:
                            new_next_hop = self.get_deflection_hop(curr_node_id, next_id, self.links)

                            if new_next_hop:
                                self.deflected_packets_count += 1
                                p['D'] = True  # SET DEFLECTION BIT
                                p['deflection_count'] += 1
                                p['deflect_anim'] = DEFLECT_ANIMATION_FRAMES
                                p['avoid_edges'].append(link_key)

                                # Recalculate Path Segment from Deflection Point
                                try:
                                    # Create temp graph excluding known failed edge to find rest of path
                                    temp_graph = self.graph.copy()
                                    if temp_graph.has_edge(curr_node_id, next_id):
                                        temp_graph.remove_edge(curr_node_id, next_id)

                                    new_path_tail = nx.shortest_path(temp_graph, new_next_hop, p['dst'])

                                    # Update Path: History + [curr, new_next] + rest
                                    # Packet is currently at 'hop' (curr_node_id).
                                    # So we keep path up to 'hop', append new path tail.
                                    # Note: shortest_path includes source (new_next_hop), so we append it.

                                    p['path'] = p['path'][:p['hop'] + 1] + new_path_tail
                                    # p['hop'] remains the same index, pointing to curr_node_id.
                                    # In next iteration, it will move to hop+1 (new_next_hop).

                                    p['pct'] = 0.0  # Reset position to center of node
                                    alive.append(p)
                                    continue
                                except:
                                    pass  # Routing failed

                        # Drop if deflection disabled or failed
                        self.dropped_packets_count += 1
                        p['drop'] = True;
                        p['drop_timer'] = DROP_ANIMATION_FRAMES;
                        p['pct'] = 1.0
...
<body>
    ...
    <div id="controls">
        <div class="slider-container">
            <label style="font-weight:bold; color:#e67e22;">Pkts/Batch:</label>
            <input type="range" min="0" max="10" step="1" value="0" id="batch-slider" oninput="updateBatch(this.value)">
            <span id="batch-val" style="font-weight:bold; color:#e67e22; margin-left:5px;">1</span>
        </div>
        <div class="slider-container">
            <label style="font-weight:bold;">Visual Speed:</label>
...
HTML_PART_2 = """
    ...
    <div id="entropy-modal">
        ...
        <div class="entropy-section">
            ...
            <div class="entropy-stat">
                <span class="entropy-stat-label">Batch Mode:</span>
                <span class="entropy-stat-value" id="ent-batch">1</span>
            </div>
            <div class="entropy-stat">
                <span class="entropy-stat-label">Inflight:</span>
                <span class="entropy-stat-value" id="ent-inflight">0</span>
            </div>
            <div class="entropy-stat">
        ...
        <div class="entropy-section">
            ...
            <div class="entropy-stat">
                <span class="entropy-stat-label">Valid EVs (Counter):</span>
                <span class="entropy-stat-value" id="ent-valid-count">0</span>
            </div>
            <div class="entropy-stat">
                <span class="entropy-stat-label">Explore Counter:</span>
                <span class="entropy-stat-value" id="ent-explore-counter">0</span>
            </div>
        </div>
    ...
    <script>
        ...
        // Mapping for slider 0-10 -> Specific values
        ...
        function openEntropyModal(nodeId, entropyInfo) {
            ...
            for(let entry of entropyInfo.buffer_entries) {
                ...
                if (icons.length > 0) status = ` (${icons.join('/')})`;
        ...
        function draw() {
            ...
            if(document.getElementById('disp-drops')) document.getElementById('disp-drops').innerText = state.dropped_packets;
            if(document.getElementById('disp-deflected')) document.getElementById('disp-deflected').innerText = state.deflected_packets;

            linkHitboxes = [];
            state.links.forEach(l => {
                const n1 = state.nodes[l.u], n2 = state.nodes[l.v];
                const x1 = n1.x/100*W, y1 = n1.y/100*H, x2 = n2.x/100*W, y2 = n2.y/100*H;
                linkHitboxes.push({u:l.u, v:l.v, bw:l.bw, x1:x1, y1:y1, x2:x2, y2:y2});

                ctx.beginPath(); ctx.moveTo(x1, y1); ctx.lineTo(x2, y2);
                ctx.strokeStyle = l.bw === 0 ? '#e74c3c' : '#bdc3c7';
                ctx.lineWidth = l.bw === 0 ? 3 : 2;
                if(l.active > 0) { ctx.lineWidth=4; ctx.strokeStyle = '#2ecc71'; }
                ctx.stroke();

                if(l.bw > 0) {
                    const mx=(x1+x2)/2, my=(y1+y2)/2;
                    ctx.fillStyle='#333'; ctx.font="10px Arial"; ctx.textAlign="center";
                    ctx.fillText(Math.round(l.util)+"%", mx, my-5);
                } else {
                     const mx=(x1+x2)/2, my=(y1+y2)/2;
            ...
            Object.entries(state.nodes).forEach(([id, n]) => {
                ...
                if(n.type !== 'host') {
                    ...
                    ctx.fillStyle='rgba(255,255,255,0.3)'; ctx.fillRect(cx-w/2+2, cy+5, w-4, 10);
                    ctx.fillStyle='#2ecc71'; 
                    const qPct = Math.min(1.0, n.q / n.cap);
                    ctx.fillRect(cx-w/2+2, cy+5, (w-4)*qPct, 10);
                } else {
                    const r=20; 
            ...
            state.packets.forEach(p => {
                ...
                if (p.drop) {
                    ...
                    // --- CHANGED COLOR LOGIC ---
                    // Deflected = Amber (#FFBF00)
                    // ACK = Green (#2ecc71)
                    // Normal = Blue (#2980b9)
                    let bgColor = '#2980b9';
                    if (p.D) {
                        bgColor = '#FFBF00'; // Amber for Deflection
                    } else if (p.ack) {
                        bgColor = '#2ecc71'; // Green for ACK
                    }

                    let scale = 1.0;

                    if (p.deflect_anim > 0) {
                        const anim_prog = (20 - p.deflect_anim) / 20;
                        scale = 1.0 + Math.sin(anim_prog * Math.PI) * 0.5;
                    }
                ...
                if (p.ecn && !p.drop) {
                    ctx.fillStyle = 'red'; ctx.beginPath(); ctx.arc(0, -8, 2, 0, 2*Math.PI); ctx.fill();
                }
...
def handle_ctrl(data):
    ...
    if cmd == 'start':
        ...
        sim.speed_multiplier = float(val)


@socketio.on('set_batch')
def handle_batch(val):
    sim.set_batch_size(val)


@socketio.on('set_mode')
def handle_mode(val):
    sim.set_deflection_mode(bool(val))


@socketio.on('set_link_bw')
def handle_link_bw(data):
    sim.update_link_bw(data['u'], data['v'], data['bw'])


@socketio.on('reset')
def handle_reset(config):
    sim.reset_topology(config)
    emit('frame', sim.get_state())
