    ...
    <script>
        ...
        function draw() {
            ...
            state.links.forEach(l => {
                ...
                // --- FIXED Y-LEVEL PLACEMENT ---
                // We do NOT want labels to follow the wire's slope. 
                // We want them to form a perfect horizontal line above/below the source node.

                const lowerNode = getLowerNode(n1, n2);
                const isN1Lower = (lowerNode.id === n1.id);

                // The source is always the lower node in our bottom-up visualization logic
                const sourcePoint = isN1Lower ? p1 : p2;

                // Calculate X position:
                // We still want the label to appear "along" the wire horizontally, so we find the x-coord
                // corresponding to a fixed vertical rise.
                // Slope: dy/dx = (p2.y - p1.y) / (p2.x - p1.x)

                const FIXED_RISE = 45; // Pixels above the source node center

                // Because standard canvas Y increases downwards, "above" means y - 45.
                const targetY = sourcePoint.y - FIXED_RISE;

                // Find targetX on the line segment at targetY
                // y = mx + c  => x = (y - c)/m
                // Or simply interpolation: t = (targetY - p1.y) / (p2.y - p1.y)

                let t = (targetY - p1.y) / (p2.y - p1.y);
                ...
                // Now we draw EXACTLY at (targetX, targetY). 
                // Since targetY is constant for all nodes on the same layer, labels will be perfectly aligned.
        ...
        function drawLabelAt(lx, ly, util, bw) {
            // Because we pass the exact center point now, we just draw around it.
            // We shift slightly to the right to avoid covering the wire itself.
            const x = lx + 22; // shift right
            const y = ly;      // keep y exact for symmetry
