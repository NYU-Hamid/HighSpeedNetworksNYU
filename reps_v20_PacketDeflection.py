...
EVS_SIZE = 65536
MAX_DEFLECTIONS = 4  # Limit bouncing to prevent infinite loops
...
class REPSHost:
    def __init__(self, id):
        ...
        # New: Global mode flag passed from Simulation
        self.use_deflection_mode = False

    def set_mode(self, use_deflection):
        self.use_deflection_mode = use_deflection
        # If switching to deflection mode, forcefully exit freezing mode
    ...
    def _get_next_ev_procedure(self):
        if self.num_valid_evs > 0:
            ...
            # Freezing Mode Logic (Only used if use_deflection_mode is False)
            offset = self.head
    ...
    def on_ack(self, ev, ecn, deflected, current_tick):
        # 1. Deflection Mode Handling (Proposal Section 4)
        if self.use_deflection_mode and deflected:
            # "Senders receiving deflection-marked ACKs blacklist that EV"
            self.deflected_ack_count += 1
            # We do NOT cache this EV. We treat it as a "bad signal" similar to ECN.
            # In fact, we should actively ensure it's not in the buffer (Blacklist).
            # For this simple implementation, ignoring it (not adding it) is sufficient
            # because the buffer rotates.
            return
        ...
        # Check Freezing Exit (Only if NOT in deflection mode)
    ...
    def on_failure_detection(self, current_tick):
        # Proposal: "Deflection... eliminates freezing mode"
        # So if we are in deflection mode, we ignore timeout/drop signals.
    ...
    def check_timeout(self, current_tick):
        if self.use_deflection_mode:
            self.is_freezing_mode = False  # Ensure we never stay frozen
            return
...
class NetworkSimulation:
    def __init__(self):
        ...
        self.current_tick = 0
        self.use_deflection_mode = False  # Default to standard, INITIALIZED BEFORE RESET
        self.reset_topology()  # Now safe to call
        ...
        with self.lock:
                ...
                if random.random() < 0.40:
                    ...
                    try:
                        ...
                        self.packets.append({
                            ...
                            'deflected': False, 'deflection_count': 0
                        })
                    except:
                        pass
            ...
            for p in self.packets:
                ...
                if p['hop'] < len(p['path']) - 1:
                    ...
                    for l in self.links:
                        if l['key'] == key:
                            # Standard behavior: if 0 bw, it's failed.
                            if l['bw'] == 0:
                                current_speed = 0  # It will stop here, waiting to be processed by queue logic below
                            else:
                ...
                if p['pct'] >= 1.0:
                    ...
                    if link_failed:
                        if self.use_deflection_mode:
                            # === DEFLECTION LOGIC ===
                            if p['deflection_count'] >= MAX_DEFLECTIONS:
                                # Too many bounces, drop it
                            ...
                            # Bounce Logic: Route from Current Node -> Dest avoiding 'next_id'
                            # In a real switch, we send it back to ingress. Here we calculate a new path.
                            try:
                                # Create a temporary graph without the failed link for pathfinding
                                temp_graph = self.graph.copy()
                                temp_graph.remove_edge(curr_node_id, next_id)

                                # Find new path from HERE to Destination
                                new_sub_path = nx.shortest_path(temp_graph, source=curr_node_id, target=p['dst'])

                                # Construct new full path:
                                # [Visited so far] + [New Path from Current]
                                # Note: p['path'][p['hop']] is current node.
                                # We replace everything after current node with new sub path.
                                # new_sub_path includes current_node as index 0.
                                p['path'] = p['path'][:p['hop']] + new_sub_path

                                # Reset percentage to 0 to start moving on new link
                                p['pct'] = 0.0
                                # Don't increment hop, we are effectively restarting from current node on new link
                                alive.append(p)
                                continue

                            except nx.NetworkXNoPath:
                                # No alternative path found, drop
                                self.dropped_packets_count += 1
                                p['drop'] = True
                                p['drop_timer'] = DROP_ANIMATION_FRAMES
                                p['pct'] = 1.0
                    ...
                    if isinstance(next_obj, REPSSwitch):
                        if p['hop'] + 2 < len(p['path']):
                            ...
                            if not next_obj.enqueue(outgoing):
                                # Queue Full Drop (Congestion, not Link Failure)
                                self.dropped_packets_count += 1
                                p['drop'] = True
                                p['drop_timer'] = DROP_ANIMATION_FRAMES
                                p['pct'] = 1.0
                                # Note: Congestion drops usually trigger ECN or timeouts, not immediate freezing for link failure
                                # But standard TCP/REPS might react. For now we assume queue drops don't trigger freezing
                                # unless it causes a timeout (handled by timeout logic).
                                alive.append(p)
                                continue
...
<body>
    ...
    <script>
        ...
        socket.on('frame', d => { 
            state = d; 
            // Sync Toggle UI to Backend State
        ...
        function draw() {
            ...
            state.packets.forEach(p => {
                ...
                if (p.drop) {
                    ...
                    else if (p.deflected) bgColor = '#8e44ad'; // Purple for Deflected
                ...
                if (p.drop) {
                    ...
                    ctx.fillText(p.deflected ? 'DEFL' : `E:${p.ev}`, 0, 0);
